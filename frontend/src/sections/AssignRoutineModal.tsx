...
function AssignRoutineModal({
  ...
  isAddExerciseDayModalOpen,
  isEditAssignExerciseModalOpen,
}: Props) {
  ...
  const handleClose = useCallback(() => {
    ...
    closeAssignRoutineModal();
  }, [assignRoutineModalOpen, closeAssignRoutineModal, isDesktopViewport]);

  useEffect(() => {
    if (!assignRoutineModalOpen || isDesktopViewport || mobileHistoryActiveRef.current) return;
    window.history.pushState({ assignRoutineMobile: true }, "", window.location.href);
    mobileHistoryActiveRef.current = true;
  }, [assignRoutineModalOpen, isDesktopViewport]);

  useEffect(() => {
    if (!assignRoutineModalOpen || isDesktopViewport) return;
    const handlePopState = () => {
      if (isAddExerciseDayModalOpen) return;
      if (isEditAssignExerciseModalOpen) return;
  ...
  }, [assignRoutineModalOpen, closeAssignRoutineModal, isAddExerciseDayModalOpen, isDesktopViewport, isEditAssignExerciseModalOpen]);

  useEffect(() => {
    if (assignRoutineModalOpen) return;
    mobileHistoryActiveRef.current = false;
  }, [assignRoutineModalOpen]);

  if (assignRoutineModalOpen && !isDesktopViewport) {
    return (
          ...
          <Box bg="white" borderBottomWidth="1px" borderColor="gray.200" px={4} pt="calc(0.75rem + env(safe-area-inset-top))" pb={3}>
            <HStack justify="space-between" align="center" spacing={3}>
              ...
              <Stack spacing={0} flex="1" align="center" mr="28px">
                <Text fontSize="16px" fontWeight="800" color="#1f2937" textAlign="center">
                  Asignar rutina a {assignRoutineStudent?.full_name || "deportista"}
                </Text>
                {assignRoutineStep === "choice" && <Text fontSize="11px" color="#667085">Paso 2 de 4: Selecciona el método de asignación</Text>}
                {assignRoutineStep === "existing_days" && <Text fontSize="11px" color="#667085">Paso 3: Selecciona inicio y fin de rutina</Text>}
                {assignRoutineStep === "existing_list" && <Text fontSize="11px" color="#667085">Paso 4: Selecciona una o más rutinas pre cargadas.</Text>}
                {assignRoutineStep === "existing_order" && <Text fontSize="11px" color="#667085">Paso 5: Ordenar rutinas seleccionadas</Text>}
                {assignRoutineStep === "existing_preview" && <Text fontSize="11px" color="#667085">Paso 6: Configurar ejercicios y días</Text>}
                {assignRoutineStep === "existing_dates" && <Text fontSize="11px" color="#667085">Paso 7: Seleccionar fechas y notas</Text>}
              </Stack>
            </HStack>
          </Box>
            ...
            <Stack spacing={4}>
              {assignRoutineStep === "choice" && (
                <Stack spacing={4} animation={`${routineStepSlide} 0.3s ease`}>
                  ...
                  <Stack spacing={3}>
                    <Box flex="1" role="group" borderWidth="1px" borderColor="gray.300" bg="white" borderRadius="16px" p={5} cursor="pointer" _hover={{ borderColor: "#f97316", bg: "#fff7ed" }} onClick={handleChooseCreateRoutineForStudent}>
                    ...
                    <Box flex="1" role="group" borderWidth="1px" borderColor="gray.300" borderRadius="16px" p={5} cursor="pointer" _hover={{ borderColor: "#f97316", bg: "#fff7ed" }} onClick={handleChooseExistingRoutineList}>
                      <Stack spacing={3} align="center" textAlign="center">
                        <Box w="44px" h="44px" borderRadius="full" bg="gray.100" display="flex" alignItems="center" justifyContent="center" color="gray.500" _groupHover={{ bg: "orange.100", color: "orange.500" }}>
                          <Box as="svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" boxSize="18px" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              ...
              {assignRoutineStep === "existing_list" && (
                <Stack spacing={3} animation={`${routineStepSlide} 0.3s ease`}>
                  {sortedRoutines.map((routine: any) => {
                    const isSelected = selectedRoutineIdsToAssign.includes(routine.id);
                    return (
                      <Box key={routine.id} p={3.5} borderWidth="1px" borderColor={isSelected ? "#f97316" : "gray.200"} bg={isSelected ? "#fff7ed" : "white"} borderRadius="12px" cursor="pointer" _hover={{ borderColor: isSelected ? "#f97316" : "gray.400" }} onClick={() => handleSelectRoutineToAssign(routine)}>
                        <Stack spacing={2}>
                          <Text color="gray.900" fontWeight="600">{routine.name}</Text>
              ...
              {assignRoutineStep === "existing_order" && (
                <Stack spacing={3} animation={`${routineStepSlide} 0.3s ease`}>
                  ...
                  <Box ref={routineOrderContainerRef}>
                    <Stack spacing={2}>
              ...
              {assignRoutineStep === "existing_preview" && selectedRoutineToAssign && (
                <Stack ref={assignRoutineSummaryListRef} spacing={4} animation={`${routineStepSlide} 0.3s ease`} onWheel={handleAssignRoutineSummaryWheel} sx={{ overscrollBehaviorY: "contain" }}>
                  <Stack ref={previewMeasureRef} spacing={4}>
                    {routineAssignBuilderDays.map((day: any) => (
                      <Box key={`assign-summary-${day.key}`} bg="white" borderWidth="1px" borderColor={isAssignPreviewOverDayLimit ? "red.300" : "#e5e7eb"} borderRadius="16px" p={4} boxShadow="0 10px 24px rgba(15, 23, 42, 0.04)">
  ...
  return (
    <Modal isOpen={assignRoutineModalOpen} onClose={handleClose} isCentered>
      ...
      >
        <ModalHeader borderBottomWidth="1px" borderColor="gray.200" py={4}>
          <HStack justify="space-between">
            ...
            <Button variant="ghost" size="sm" color="gray.400" _hover={{ bg: "gray.100", color: "gray.700" }} onClick={handleClose}>×</Button>
          </HStack>
        </ModalHeader>
        <Box
          h={stepBodyHeight}
