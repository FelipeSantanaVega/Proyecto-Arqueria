...
function CreateRoutineModal({
  ...
  isRoutineCreateAddExerciseModalOpen,
  isCreateExerciseModalOpen,
  isRoutineCreateEditExerciseModalOpen,
}: Props) {
  const [isDesktopViewport] = useMediaQuery("(min-width: 48em)");
  const mobileHistoryActiveRef = useRef(false);

  const mobileTitle = editingRoutineId ? "Editar rutina" : "Nueva Rutina";

  const handleClose = useCallback(() => {
    if (!createRoutineModalOpen) {
      closeCreateRoutineModal();
      return;
    }
    ...
    if (mobileHistoryActiveRef.current) {
      window.history.back();
      return;
    }
    closeCreateRoutineModal();
  }, [closeCreateRoutineModal, createRoutineModalOpen, isDesktopViewport]);

  useEffect(() => {
    if (!createRoutineModalOpen || isDesktopViewport || mobileHistoryActiveRef.current) return;
    window.history.pushState({ createRoutineMobile: true }, "", window.location.href);
    mobileHistoryActiveRef.current = true;
  }, [createRoutineModalOpen, isDesktopViewport]);

  useEffect(() => {
    if (!createRoutineModalOpen || isDesktopViewport) return;
    const handlePopState = () => {
      if (isRoutineCreateAddExerciseModalOpen) return;
      if (isCreateExerciseModalOpen) return;
      if (isRoutineCreateEditExerciseModalOpen) return;
      if (!mobileHistoryActiveRef.current) return;
      mobileHistoryActiveRef.current = false;
      closeCreateRoutineModal();
    };
    window.addEventListener("popstate", handlePopState);
    return () => window.removeEventListener("popstate", handlePopState);
  }, [closeCreateRoutineModal, createRoutineModalOpen, isCreateExerciseModalOpen, isDesktopViewport, isRoutineCreateAddExerciseModalOpen, isRoutineCreateEditExerciseModalOpen]);

  useEffect(() => {
    if (createRoutineModalOpen) return;
    mobileHistoryActiveRef.current = false;
  }, [createRoutineModalOpen]);

  if (createRoutineModalOpen && !isDesktopViewport) {
    return (
          ...
          <Box bg="white" borderBottomWidth="1px" borderColor="gray.200" px={4} pt="calc(0.75rem + env(safe-area-inset-top))" pb={3}>
            <HStack justify="space-between" align="center" spacing={3}>
              ...
              <Stack spacing={0} flex="1" align="center" mr="28px">
                <Text fontSize="16px" fontWeight="800" color="#1f2937">
                  {mobileTitle}
                </Text>
                <Text fontSize="11px" color="#667085">
                  Paso {routineModalStep + 1}
                </Text>
              </Stack>
            </HStack>
          </Box>
          <Box flex="1" overflowY="auto" px={3.5} py={4} pb="calc(2rem + env(safe-area-inset-bottom))">
            <MobileRoutineFlow
              ...
              closeCreateRoutineModal={handleClose}
            />
          </Box>
        </Flex>
      </Box>
  ...
  return (
    <Modal isOpen={createRoutineModalOpen} onClose={handleClose} isCentered>
              ...
              <Stack ref={routineStepRef} spacing={6} animation={`${routineStepSlide} 0.3s ease`} px={2} pt={2} pb={0} w="full" maxW="420px" mx="auto">
                ...
                <HStack spacing={3} justify="center" wrap="wrap">
                  ...
                  >
                    ...
                    <Button variant="ghost" color="gray.600" _hover={{ bg: "transparent", color: "gray.800" }} onClick={handleClose}>
                    Cancelar
                  </Button>
                </HStack>
              </Stack>
            ...
            {routineModalStep === 1 && (
              <Stack ref={routineStepRef} spacing={5} animation={`${routineStepSlide} 0.3s ease`} px={2} pt={2} pb={4} w="full" maxW="700px" mx="auto">
                ...
                <HStack spacing={3} justify="flex-end" wrap="wrap">
                  ...
                  <Button bg="white" color="black" borderColor="gray.300" borderWidth="1px" _hover={{ bg: "gray.100" }} onClick={handleClose}>Cancelar</Button>
                </HStack>
              </Stack>
            )}
            {routineModalStep === 2 && (
            ...
            {routineModalStep === 3 && (
              <Stack ref={routineStepRef} spacing={4} animation={`${routineStepSlide} 0.3s ease`}>
                <HStack justify="space-between" px={6} py={4} borderBottomWidth="1px" borderColor="gray.200">
                  ...
                  <Button variant="ghost" size="sm" color="gray.500" onClick={handleClose}>×</Button>
                </HStack>
...
function MobileRoutineFlow(props: any) {
  const {
    routineModalStep,
    routineStepRef,
    editingRoutineId,
  ...
  return (
    <>
      {routineModalStep === 0 && (
        <Stack ref={routineStepRef} spacing={4} px={0.5}>
          <Box bg="white" borderWidth="1px" borderColor="#e5e7eb" borderRadius="16px" p={4} boxShadow="0 10px 24px rgba(15, 23, 42, 0.04)">
            <Stack spacing={4}>
              <Stack spacing={2} align="center" textAlign="center">
                <Box w="52px" h="52px" borderRadius="full" bg="#fff7ed" display="flex" alignItems="center" justifyContent="center" color="#f97316">
                  <Box as="svg" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" boxSize="20px" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                    <path d="M12.83 2.18a2 2 0 0 0-1.66 0L2.6 6.08a1 1 0 0 0 0 1.83l8.58 3.91a2 2 0 0 0 .83.18 2 2 0 0 0 .83-.18l8.58-3.9a1 1 0 0 0 0-1.831z" />
                    <path d="M16 17h6" />
                    <path d="M19 14v6" />
      ...
      {routineModalStep === 1 && (
        <Stack ref={routineStepRef} spacing={4} px={0.5}>
          <Box bg="white" borderWidth="1px" borderColor="#e5e7eb" borderRadius="16px" p={4} boxShadow="0 10px 24px rgba(15, 23, 42, 0.04)">
            {routineAssignStudentId && !editingRoutineId ? (
              <Stack spacing={4}>
                <Heading size="sm" textAlign="center">Selecciona el rango de la rutina</Heading>
              ...
              <Stack spacing={4} align="center" textAlign="center">
                <Heading size="sm">Elige cuántos días tendrá la rutina</Heading>
                <VerticalDayWheelPicker value={routineDayCount} min={1} max={7} onChange={setRoutineDayCount} />
              </Stack>
            )}
          </Box>
      ...
      {routineModalStep === 2 && (
        <Stack ref={routineStepRef} key={`${currentRoutineDayKey || "dia"}-${routineDayCursor}`} spacing={4} px={0.5}>
          <Box bg="white" borderWidth="1px" borderColor="#e5e7eb" borderRadius="16px" p={4} boxShadow="0 10px 24px rgba(15, 23, 42, 0.04)">
            <HStack justify="space-between" align="start" mb={4}>
              <Heading size="sm">{currentRoutineDayLabel || "Día"}</Heading>
              <Text fontSize="sm" color="gray.500" fontWeight="500">
                {currentRoutineDayTotalArrows} flechas
              </Text>
            </HStack>
            <InputGroup>
              <InputLeftElement pointerEvents="none" color="gray.500"><SearchIcon boxSize={3.5} /></InputLeftElement>
              <Input value={routineExerciseSearch} onChange={(e) => setRoutineExerciseSearch(e.target.value)} placeholder="Buscar ejercicios" bg="white" borderColor="gray.300" borderRadius="10px" _hover={{ borderColor: "gray.400" }} _focus={{ borderColor: "#f97316", bg: "white" }} />
            </InputGroup>
            <Stack spacing={2} maxH="46vh" overflowY="auto" pr={1} mt={4}>
              {filteredRoutineExercises.map((ex: any) => {
                const selectedForCurrentDay = !!currentRoutineDayKey && (routineExercisesByDay[currentRoutineDayKey] || []).includes(ex.id);
                return (
                  <Box key={ex.id} p={3} borderWidth="1px" borderColor={selectedForCurrentDay ? "#f97316" : "gray.200"} borderRadius="12px" bg={selectedForCurrentDay ? "#fff7ed" : "white"} cursor="pointer" _hover={{ borderColor: selectedForCurrentDay ? "#f97316" : "gray.400" }} onClick={() => currentRoutineDayKey && toggleRoutineExerciseForDay(currentRoutineDayKey, ex.id)}>
                    <HStack justify="space-between" align="center" spacing={3}>
                      <Stack spacing={0.5} minW={0}>
                        <Text fontSize="sm" noOfLines={1}>{ex.name}</Text>
                        <Text fontSize="xs" color="gray.500">{ex.arrows_count} flechas</Text>
          ...
          <Button variant="outline" borderColor="gray.300" borderRadius="lg" color="gray.800" _hover={{ borderColor: "gray.500" }} onClick={() => setCreateModalOpen(true)}>
            <HStack justify="center" spacing={2}>
              <Image src={bowIconUrl} alt="Bow icon" boxSize="18px" />
              <Text>Crear ejercicio</Text>
            </HStack>
      ...
      {routineModalStep === 3 && (
        <Stack ref={routineStepRef} spacing={4} px={0.5}>
          <Stack spacing={3} ref={routineSummaryListRef} maxH="unset" onWheel={handleRoutineSummaryWheel}>
            {routineBuilderDays.map((day: any) => (
              <Box key={`summary-${day.key}`} bg="white" borderWidth="1px" borderColor="#e5e7eb" borderRadius="16px" p={4} boxShadow="0 10px 24px rgba(15, 23, 42, 0.04)">
                <HStack justify="space-between" align="baseline">
                  <Text color="gray.800" fontWeight="700">{day.label}</Text>
                  <Text fontSize="sm" color="gray.500">{getSummaryDayArrows(routineExercisesByDay[day.key] || [], day.key, routineCreateExerciseOverridesByDay)} flechas</Text>
                </HStack>
                <Stack spacing={2} mt={3}>
                  {(routineExercisesByDay[day.key] || []).map((exerciseId: number, itemIndex: number) => {
                    ...
                    const override = routineCreateExerciseOverridesByDay[day.key]?.[getRoutineEntryKey(exerciseId, itemIndex)];
                    return (
                      <Box key={`summary-ex-${day.key}-${itemIndex}-${exerciseId}`} borderWidth="1px" borderColor="gray.200" borderRadius="12px" p={3}>
            ...
            {routineDayCount < routineDayInitialLimit && <Button variant="ghost" color="gray.600" _hover={{ bg: "gray.100", color: "gray.700" }} onClick={handleAddRoutineDay}>Agregar día</Button>}
          </Stack>
          {createRoutineError && <Alert status="error" borderRadius="md"><AlertIcon />{createRoutineError}</Alert>}
          <Stack spacing={3}>
            <Button variant="outline" borderColor="gray.300" onClick={() => { setRoutineDayCursor(Math.max(routineBuilderDays.length - 1, 0)); setRoutineModalStep(2); }}>Volver</Button>
        ...
        <Stack ref={routineStepRef} spacing={4} px={0.5}>
          <Box bg="white" borderWidth="1px" borderColor="#e5e7eb" borderRadius="16px" p={4} boxShadow="0 10px 24px rgba(15, 23, 42, 0.04)">
            <Stack spacing={4}>
              <FormControl>
                <FormLabel>Objetivo</FormLabel>
                <Input value={assignmentObjective} onChange={(e) => setAssignmentObjective(e.target.value)} placeholder="Ej: Determinante" />
              </FormControl>
              <FormControl>
                <FormLabel>Notas del profesor (opcional)</FormLabel>
                <Textarea value={assignmentProfessorNotes} onChange={(e) => setAssignmentProfessorNotes(e.target.value)} placeholder="Escribe observaciones para el deportista..." minH="110px" resize="vertical" />
